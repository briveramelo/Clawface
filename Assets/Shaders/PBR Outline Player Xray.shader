...
Shader "PBR Outline Player Xray"
{
	Properties
	{
		...
		_Color1("Color 1", Color) = (0.875,0.04503676,0.04503676,0)
		_Color2("Color 2", Color) = (0.875,0.04503676,0.04503676,0)
		_XrayNoise("Xray Noise", 2D) = "bump" {}
		_NoiseSpeed("Noise Speed", Range( 0 , 5)) = 0.3393778
		_NoiseSize("Noise Size", Range( 0 , 2)) = 0
	}
	...
	SubShader
	{
		...
		{
			float2 panner112 = ( i.texcoord_0 + ( _Time.x * _NoiseSpeed ) * float2( 1,1 ));
			float4 lerpResult121 = lerp( _Color1 , _Color2 , float4( UnpackNormal( tex2D( _XrayNoise, panner112 ) ) , 0.0 ));
